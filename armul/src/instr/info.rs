...
use crate::{
    ...
    instr::{
        ...
        SpecialOperand, TransferKind, TransferOperand,
    },
};
    ...
    fn from_data_operand(value: DataOperand) -> Vec<Self> {
        match value {
            ...
            DataOperand::Register(register, shift) => match shift.shift_amount {
                ShiftAmount::Constant(0) if shift.shift_type != ShiftType::RotateRightExtended => {
                    vec![PrettyArgument::Register {
                        register,
                        negative: false,
                        write_back: false,
                    }]
...
impl PrettyInstr {
    pub fn new(address: u32, cond: Cond, instr: Instr) -> Self {
        let (opcode_prefix, opcode_suffix) = opcode_parts(instr);
        Self {
            opcode_prefix,
            cond: cond.to_string(),
            opcode_suffix,
            args: instr_args(address, instr),
        }
    }
}

fn opcode_parts(instr: Instr) -> (String, String) {
    match instr {
...
fn instr_args(address: u32, instr: Instr) -> Vec<PrettyArgument> {
    match instr {
        ...
        Instr::SingleTransfer {
            ...
            let offset = match offset {
                TransferOperand::Constant(0) => Vec::new(),
                TransferOperand::Constant(n) => vec![PrettyArgument::Constant {
                    negative: !offset_positive,
                    value: n as u32,
                    style: ConstantStyle::Unknown,
                }],
                TransferOperand::Register(register, shift) => {
                    if shift.shift_amount == ShiftAmount::Constant(0)
                        && shift.shift_type != ShiftType::RotateRightExtended
                    {
                        vec![PrettyArgument::Register {
                            register,
                            negative: !offset_positive,
            ...
            pretty_transfer(write_back, pre_index, data_register, base_register, offset)
        }
        Instr::SingleTransferSpecial {
            write_back,
            offset_positive,
            ...
            let offset = match offset {
                SpecialOperand::Constant(0) => Vec::new(),
                SpecialOperand::Constant(n) => vec![PrettyArgument::Constant {
                    negative: !offset_positive,
                    value: n as u32,
                    style: ConstantStyle::Unknown,
                }],
...
fn pretty_transfer(
    ...
    base_register: Register,
    offset: Vec<PrettyArgument>,
) -> Vec<PrettyArgument> {
    let mut args = vec![PrettyArgument::Register {
        register: data_register,
        negative: false,
        write_back: false,
    }];
    if pre_index {
        args.push(PrettyArgument::Address {
            base_register,
            operands: offset,
            write_back,
        });
